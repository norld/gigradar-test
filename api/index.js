...
// api/lib/middleware/validation.ts
var MAX_PAYLOAD_SIZE = 3 * 1024 * 1024;
async function validatePayloadSize(req, reply) {
...
// api/lib/middleware/timeout.ts
var TIMEOUT_WARNING_THRESHOLD = 8e3;
function trackRequestTimeout(req, reply) {
...
// api/lib/middleware/logger.ts
var import_pino = __toESM(require("pino"));

// api/lib/config/env.ts
var import_dotenv = __toESM(require_main());
var import_zod = require("zod");
(0, import_dotenv.config)();
var envSchema = import_zod.z.object({
  TELEGRAM_BOT_TOKEN: import_zod.z.string().min(1, "TELEGRAM_BOT_TOKEN cannot be empty").regex(/^\d+:[A-Za-z0-9_-]+$/, "TELEGRAM_BOT_TOKEN format is invalid").optional(),
  OPEN_AI_TOKEN: import_zod.z.string().min(1, "OPEN_AI_TOKEN cannot be empty").startsWith("sk-", 'OPEN_AI_TOKEN must start with "sk-"').optional(),
  LOG_LEVEL: import_zod.z.enum(["debug", "info", "warn", "error"]).optional(),
  NODE_ENV: import_zod.z.enum(["development", "production", "test"]).optional(),
  PORT: import_zod.z.coerce.number().int().positive().max(65535).optional()
});
function getRequiredOrTest(value, key) {
  const isTest = process.env.NODE_ENV === "test";
  if (value === void 0 && !isTest) {
...
// api/lib/services/openai.ts
var OpenAIService = class {
  client;
  model = "gpt-4o";
  // Supports both text and vision
  maxHistoryLength = 20;
  // Last 20 messages as per requirements
  constructor() {
   ...
   */
  truncateHistory(messages, maxLength = this.maxHistoryLength) {
...
// api/lib/services/messageProcessor.ts
var SYSTEM_PROMPT = `You are a helpful AI assistant powered by GPT-4. You can help with:
- Answering questions
...
var messageHistory = /* @__PURE__ */ new Map();
...
// api/lib/routes/webhook.ts
var webhookBodySchema = import_zod2.z.object({
  update_id: import_zod2.z.number().int().positive(),
  message: import_zod2.z.object({
    message_id: import_zod2.z.number().int().positive(),
    from: import_zod2.z.object({
      id: import_zod2.z.number().int().positive(),
      is_bot: import_zod2.z.boolean(),
      first_name: import_zod2.z.string()
    }),
    chat: import_zod2.z.object({
      id: import_zod2.z.number().int().positive(),
      type: import_zod2.z.enum(["private", "group", "supergroup", "channel"])
    }),
    date: import_zod2.z.number().int().nonnegative()
  }).optional()
});
...
async function registerWebhookRoutes(fastify2) {
  fastify2.post(
    "/webhook",
    {
      schema: {
...
async function registerHealthRoutes(fastify2) {
  fastify2.get(
    "/health",
    {
      schema: {
...
function getServer() {
  ...
  const server = (0, import_fastify.default)({
    logger: false,
    // Disable built-in logger, using custom Pino logger
    disableRequestLogging: true,
    // Disable request logging, using custom middleware
    trustProxy: true
